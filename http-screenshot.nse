...
action = function(host, port)
	...
	-- Execute the shell command phantomjs <url> <filename>
    local cmd = "phantomjs --ignore-ssl-errors=true /tmp/screenshot.js " .. prefix .. "://" .. host.ip .. " " .. filename .. " 2> /dev/null   >/dev/null"

	local ret = os.execute(cmd)

	-- If the command was successful, print the saved message, otherwise print the fail message
	local result = "failed (verify phantomjs is in your path)"
